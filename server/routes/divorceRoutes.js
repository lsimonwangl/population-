...
router.get('/total/:yyyymm', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      'SELECT SUM(divorce_pair) as total FROM records WHERE statistic_yyyymm = ?',
      [yyyymm]
    );

    if (rows[0].total === null) {
      return res.status(404).json({ message: `找不到 ${yyyymm} 月的離婚資料` });
    }
...
router.get('/trend/:site_id/:village', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT statistic_yyyymm as month, SUM(divorce_pair) as total
       FROM records
